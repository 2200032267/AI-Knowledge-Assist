...
Drag-drop PDFs. Saved to backend/data/uploads/ on disk.

Backend RAG

/ask endpoint uses _VECTOR_STORE RAM with Counter vectors + cosine similarity.

Frontend RAG

Document mode uses browser localStorage with 384-dim embeddings for offline Q&A.

Agent Mode

Detects "summarize", "notes", "explain" keywords to prepend instructions to LLM.

Chat History

Per-chat JSON files in backend/data/history/ synced to browser localStorage.

Reprocess

POST /rag/reprocess loads PDF from disk and rebuilds RAM vectors after restart.

Settings

Tune chunk_size, chunk_overlap, top_k via /settings endpoint.

Responsive UI

Built with React + Tailwind CSS for seamless mobile and desktop experience.
...
MemoryError → Process crash → Auto-restart → RAM empty

Render Disk

1 GB

OSError on upload → 500 error, existing data safe

localStorage

5-10 MB

QuotaExceededError → Silent fail, doc mode breaks
...
Render Sleep: After 15min inactivity, RAM wiped. _VECTOR_STORE lost. PDFs remain. Use /rag/reprocess to restore.
...
1_upload_page.png – PDF upload with drag-drop
2_chat_with_pdf.png – RAG Q&A with source citations
3_agent_mode.png – Summary/notes generation
4_settings_page.png – RAG tuning + storage display
5_mobile_view.png – Responsive design
👤 Author
NEDULLA VIGHNESH
